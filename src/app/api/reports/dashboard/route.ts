...
// The production orders table uses customer_order_date/created_at rather than
// the legacy order_date field. Keep one canonical reporting date expression so
// the dashboard works against the current production schema.
const ORDER_REPORT_DATE = `COALESCE(o.customer_order_date, o.created_at)`;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [kpis, monthly, products, customers, pending, exposure] = await Promise.all([
      pool.query(`SELECT
        (SELECT COUNT(*) FROM orders WHERE deleted_at IS NULL AND COALESCE(customer_order_date, created_at) BETWEEN $1::date AND $2::date)::int orders,
        (SELECT COALESCE(SUM(total_amount),0) FROM orders WHERE deleted_at IS NULL AND status NOT IN ('draft','canceled') AND COALESCE(customer_order_date, created_at) BETWEEN $1::date AND $2::date) sales,
        (SELECT COALESCE(SUM(oi.quantity*(oi.unit_sale_price-oi.unit_purchase_price)),0) FROM order_items oi JOIN orders o ON o.id=oi.order_id WHERE o.deleted_at IS NULL AND o.status NOT IN ('draft','canceled') AND ${ORDER_REPORT_DATE} BETWEEN $1::date AND $2::date) profit,
