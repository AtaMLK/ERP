...
export async function matchProduct(client:PoolClient, reference:string, customerId?:number):Promise<Match[]> {
  ...
  const r=await client.query(`SELECT p.id,p.sku,COALESCE(p.product_name_en,p.name) name,p.description,
    CASE WHEN lower(p.sku)=lower($1) THEN 100 WHEN regexp_replace(lower(p.sku),'[^a-z0-9]','','g')=$2 THEN 98 WHEN $2 LIKE '%'||regexp_replace(lower(p.sku),'[^a-z0-9]','','g')||'%' THEN 97 ELSE 0 END score,
    ...
    UNION ALL SELECT p.id,p.sku,COALESCE(p.product_name_en,p.name),p.description,96,'alias'
      FROM product_aliases a JOIN products p ON p.id=a.product_id WHERE p.deleted_at IS NULL AND regexp_replace(lower(a.alias),'[^a-z0-9]','','g') <> '' AND $2 LIKE '%'||regexp_replace(lower(a.alias),'[^a-z0-9]','','g')||'%'
    UNION ALL SELECT p.id,p.sku,COALESCE(p.product_name_en,p.name),p.description,96,'customer_sku'
      FROM customer_products cp JOIN products p ON (cp.master_product_id=p.id OR lower(p.sku)=lower(cp.supplier_sku))
    ...
    LIMIT 100`,[raw,n,customerId||0,`%${raw}%`]);
