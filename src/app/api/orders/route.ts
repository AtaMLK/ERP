...
import { orderCreateSchema, validationMessage } from '@/lib/validation';

export async function GET(req: NextRequest) {
  try {
    const user = await getSession(req); requirePermission(user, 'orders:read');
    const url = new URL(req.url); const status = url.searchParams.get('status'); const q = url.searchParams.get('q');
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!parsed.success) return Response.json({success:false,error:validationMessage(parsed.error)},{status:400});
    ...
    if (!['EUR','USD','TRY'].includes(currency)) throw new Error('Unsupported currency');
    ...
    for (const item of body.items) {
      ...
      const line = qty * sale; total += line; const margin = sale ? ((sale-purchase)/sale)*100 : 0;
