...
async function main(){
 ...
 try{await c.query('BEGIN');
  ...
  let imported=0;
  for(const x of products){const sku=String(x.code||x.sku||'').trim();if(!sku)continue;const purchase=num(x.purchase_price);const sale=num(x.unit_price??x.sale_price);const material=materials(x.materials??x.material);const desc=[material,x.product_id,x.group_name].filter(Boolean).join(' | ');const name=String(x.product_name||x.name||x.product_name_en||sku).trim();const r=await c.query(`INSERT INTO products(name,product_name_en,sku,description,product_family,category,supplier_id,purchase_price,sale_price,currency,status) VALUES($1,$1,$2,$3,$4,$5,$6,COALESCE($7,0),COALESCE($8,0),$9,'ACTIVE') ON CONFLICT(sku) DO UPDATE SET name=EXCLUDED.name,product_name_en=EXCLUDED.product_name_en,description=EXCLUDED.description,product_family=EXCLUDED.product_family,category=EXCLUDED.category,supplier_id=EXCLUDED.supplier_id,purchase_price=EXCLUDED.purchase_price,sale_price=EXCLUDED.sale_price,currency=EXCLUDED.currency,updated_at=now() RETURNING id`,[name,sku,desc,x.group_name||x.product_id||null,x.category||null,supplier.id,purchase,sale,x.currency||'EUR']);const id=r.rows[0].id;imported++;
   for(const alias of [x.source_code,x.code].filter(Boolean)){await c.query(`INSERT INTO product_aliases(product_id,alias,source) VALUES($1,$2,'business-import') ON CONFLICT(product_id,alias) DO NOTHING`,[id,String(alias)]);}
   if(purchase!=null||sale!=null)await c.query(`INSERT INTO product_price_history(product_id,purchase_price,sale_price,margin_percent,currency,valid_from) VALUES($1,$2,$3,CASE WHEN $3 IS NOT NULL AND $3<>0 THEN ROUND((($3-COALESCE($2,0))/$3)*100,2) ELSE NULL END,$4,now())`,[id,purchase,sale,x.currency||'EUR']);
  }
