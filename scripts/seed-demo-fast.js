...
async function columns(client, table) {
  ...
  const value = new Set(r.rows.map((x) => x.column_name));
...
async function main() {
  ...
  try {
    ...
    if (!admin) throw new Error('No active user exists. Run complete-demo-data or create an admin user first.');

    console.log('Seeding demo suppliers/customers/products...');
    const supplierIds = [];
    for (let i = 1; i <= 3; i++) {
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      const exists = (await client.query(`SELECT id FROM customer_inquiries WHERE inquiry_number=$1 LIMIT 1`, [number])).rows[0];
      if (exists) continue;
    ...
    console.log('Seeding orders, shipments, invoices and payments...');
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      if (i % 5 === 0) console.log(`Orders seeded: ${i}/20`);
    }
