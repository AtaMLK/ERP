...
async function main(){
 ...
 try{
  const cols=await c.query(`SELECT table_name,column_name,data_type FROM information_schema.columns WHERE table_name IN ('users','roles') AND column_name='id' ORDER BY table_name`);
  assert(cols.rows.length===2 && cols.rows.every(r=>r.data_type==='integer'),'users.id and roles.id must both be integer');
  ...
  const roles=await c.query(`SELECT name FROM roles WHERE name IN ('Admin','Sales','Purchase','Accountant','Export')`); assert(roles.rows.length===5,'Canonical roles are incomplete');
  const reports=await c.query('SELECT COUNT(*)::int count FROM report_definitions WHERE active=true'); assert(Number(reports.rows[0].count)>=19,'Report catalog is incomplete');
  const permissions=await c.query(`SELECT COUNT(*)::int count FROM permissions WHERE resource='reports' AND action='read'`); assert(Number(permissions.rows[0].count)>=1,'reports:read permission missing');
